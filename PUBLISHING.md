# Publishing sm-server to PyPI

`sm-server` publishes via **PyPI Trusted Publishing** — no API tokens. You tell PyPI
once that this repo's `release.yml` workflow may publish; after that, pushing a
version tag builds and uploads automatically.

> Do this only when `sm-server` is meant to be **public**.
...
2. Go to **https://pypi.org/manage/account/publishing/** → "Add a pending
   publisher" (use *pending*, because the `sm-server` project doesn't exist on PyPI
   yet — the first publish creates it).
3. Fill the form with **exactly** these values:

   | Field | Value |
   |-------|-------|
   | PyPI Project Name | `sm-server` |
   | Owner | `Sharathvc23` |
   | Repository name | `sm-server` |
...
`pip install sm-server` works for everyone.
